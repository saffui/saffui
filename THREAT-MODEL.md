...
| T-LOG-7 | The authorization endpoint made to send a browser where no client registered: a link on the provider's own name that lands on a page of somebody else's | TA-1 | A refusal is sent only to a redirect the client registered, matched whole, and shown otherwise; a request object that cannot be read is believed in nothing, its redirect included, so its refusal goes to the redirect beside it only when that one is registered (`crates/services/src/oidc/authorize.rs:839`, `:236`, `:200`) |
