...
| T-LOG-6 | A password typed into the name box, read back out of the failure counts, or out of the notes of a login in progress, in a database dump, or one name matched across realms there | TA-7 | A typed name is kept as a MAC under a key the KEK is expanded to for that use alone, which the database never holds, so the counts read without the KEK test no guess (`crates/auth/src/login/throttle.rs:86`, `crates/crypto/src/envelope.rs:244`, `:113`); the tenant and the realm are keyed with the name, so one name typed in two realms leaves two digests nobody can match without the KEK (`crates/auth/src/login/throttle.rs:242`); a login notes the name it is weighed under in that form, and a device token is sealed under it (`crates/auth/src/login/browser.rs:287`, `:513`); the key is derived once when the process starts and handed to every door, so each counts a name where the others do (`crates/outbound/src/sealing.rs:37`, `crates/saffui/src/main.rs:421`) |
