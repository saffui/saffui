...
| T-EDGE-4 | The server made to fetch inside its own network on somebody's say so | TA-3, TA-4 | One builder for every outbound HTTP call, scheme by policy and address by resolver, no redirect followed (`crates/outbound/src/egress.rs:104`), including the sinks a client's own registration names (`crates/server/src/api/rest/endpoints/protocol/backchannel.rs:20`, `crates/server/src/api/rest/endpoints/protocol/ciba.rs:605`); the relay a realm names for its mail is weighed under the same policy before it is dialled, the relay probe included (`crates/outbound/src/smtp.rs:164`) |
