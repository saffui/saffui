...
| T-TOK-4 | The proof key stripped in flight | TA-1 | A verifier presented for a code carrying no challenge is refused whoever the client is (`crates/services/src/oidc/grant.rs:672`) |
| T-TOK-5 | A refresh token stolen and renewed | TA-1, TA-4 | Rotation by default, compared and rotated in one write, and a replay closes the family while leaving the sign in alive (`crates/services/src/oidc/grant.rs:1479`, `crates/store/src/providers/protocol/sessions.rs:547`, `crates/services/src/oidc/grant.rs:1533`) |
| T-TOK-15 | One person holding more offline grants than the realm allows, each standing long after any login, by collecting them over a device | TA-2 | Every grant that writes an offline grant makes room under the realm's cap, the device poll as the code and decoupled grants, ending the oldest (`crates/services/src/oidc/grant.rs:1266`) |
...
| T-TOK-14 | An identity token a client registered to receive encrypted handed out readable by the device or the decoupled grant, for whoever reads the answer on its way or wherever the client keeps it | TA-6 | Every grant that mints an identity token wraps it for a client that registered encryption, the device and decoupled polls as the code grant, and one that cannot be encrypted to is refused rather than answered in the clear (`crates/services/src/oidc/grant.rs:1054`, `:1277`, `crates/services/src/oidc/encryption.rs:120`) |
...
| T-TOK-18 | An exchange let through unasked because the policy meant to decide it was named in part, or in a shape nobody reads | TA-4 | A policy named in part, or with a part nobody can read, refuses the exchange; named whole it is asked, named not at all it is no policy (`crates/services/src/oidc/grant.rs:1836`) |
| T-TOK-9 | Delegation grown wider or deeper than it was given | TA-4 | Scope intersected and never widened, a ceiling of five links, and a bound token cannot be exchanged (`crates/services/src/oidc/grant.rs:1934`, `:1804`, `:1726`) |
| T-TOK-17 | Delegation kept alive past the token it acts on, a token about to end exchanged for a fresh one and the result exchanged again | TA-4 | What an exchange mints ends no later than the token it was shown, and a token with nothing left buys nothing (`crates/services/src/oidc/grant.rs:2004`) |
...
| T-TOK-16 | An exchange pointed at any audience although the operator bounded it, the bound written as a list or in a shape nobody reads | TA-4 | The bound is read in every shape it is written in, a list as the string it spells, and one that names nobody allows nobody but the client itself (`crates/services/src/oidc/grant.rs:1954`) |
| T-TOK-11 | A person switched off still signed in through the logins she held: her browser minting codes at every client, a code or a token from before still honoured, and all of it revived when she is switched back on | TA-2 | Every write that switches a person off ends every login she holds, offline grants included (`crates/services/src/directory.rs:24`); and a login stands only while its person is switched on, read in the same statement as the login, at the token gate behind every door, at the reuse of a signed-in browser and at the code exchange (`crates/store/src/providers/protocol/sessions.rs:98`, `crates/services/src/token/mod.rs:294`, `crates/services/src/oidc/authorize.rs:748`, `crates/services/src/oidc/grant.rs:456`) |
