...
function findServiceModule(call: unknown, name: string): string {
  for (const [path, exported] of Object.entries(SERVICES)) {
    if (Object.values(exported).includes(call)) return basename(path, ".ts");
  }
  throw new Error(`${name} is not exported by an account console service`);
}
...
export async function keepAnswer<A extends unknown[], R>(
  ...
  if (answer !== undefined) {
    kept.push({
      ...
      module: findServiceModule(call, call.name),
      call: call.name,
...
/// Writes what one contract file kept as TypeScript in which every answer has to
/// fit its service's return type. Extra fields fit; a missing, renamed or retyped
/// one does not, and `vue-tsc` says which.
