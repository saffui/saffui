...
(`crates/services/src/account/api.rs:87`, `:112`, `:131`, `:150`). The doorbell
of a decoupled sign-in admits its bearer by the same function, so no client
answers there for the person it asked about (`crates/services/src/account/api.rs:178`).
...
| T-TOK-5 | A refresh token stolen and renewed | TA-1, TA-4 | Rotation by default, compared and rotated in one write, and a replay closes the family while leaving the sign in alive (`crates/services/src/oidc/grant.rs:1468`, `crates/store/src/providers/protocol/sessions.rs:533`, `crates/services/src/oidc/grant.rs:1522`) |
