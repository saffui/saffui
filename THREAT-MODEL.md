...
- the mail, SMS, WhatsApp and USSD providers a realm sends through, and every
  upstream identity provider a realm federates with;
...
|---|---|---|
| A.DATABASE | No untrusted party holds a database session, and the role the server serves as is not raised above row security while it runs | CJ-2 entirely: a role that bypasses row level security reads every realm while every policy still reads as if it applied. The role is checked when the server starts (T-DB-4), not after |
...
| A.EDGE | The deployment bounds request rates in front of the data plane | T-EDGE-1: nothing inside this product limits how fast anything may be asked of it, but for passwords, and short codes at the device page, failed from one address |
...
| Provider secrets sealed per realm | Mail, SMS, WhatsApp, USSD and federation credentials, and the key a realm proves itself to its carrier with, each usable elsewhere | High |
...
`crates/server/src/api/config.rs:542`).
...
(`crates/services/src/account/api.rs:85`, `:110`, `:129`, `:148`). The doorbell
of a decoupled sign-in admits its bearer by the same function, so no client
answers there for the person it asked about (`crates/services/src/account/api.rs:176`).
...
not divided by itself (`crates/store/src/tenancy.rs:17`,
...
(`crates/pgcore/src/database.rs:98`): `verify-full` checks the server's
...
falls back to the clear without a word (`crates/pgcore/src/database.rs:307`).
A full pool refuses after a bounded wait instead of holding requests for ever
(`:177`), and a pooled transaction nobody talks to is ended by the server with
its locks (`:191`). A pooler's address, when one is named, is settled by the
same policy (`:121`), and the served pool crosses it with no startup options,
the bound on an idle transaction riding in each transaction instead
(`crates/store/src/tenancy.rs:391`). The refusal is a 503 in each surface's
own words, and the guards give it as such rather than as a missing token, which
would sign a console out (`crates/server/src/error.rs:30`, `:40`). A store that
fails once the realm is found is a 500 there, and only the token's own refusals
answer as a missing token (`:49`, `:55`, `:63`).
...
(`crates/store/src/tenancy.rs:329`).
...
(`crates/scheduler/src/jobs.rs:87`), outbox delivery (`:168`), and federation
...
| Id | Threat | Agent | What answers it |
|---|---|---|---|
| T-LOG-1 | A form posted from another site, riding the browser's cookie, answering somebody's sign in or their consent | TA-1 | The form carries a value sealed for that login, weighed before the flow runs, so a forged form spends nothing, not even one password attempt against an account (`crates/server/src/api/rest/endpoints/protocol/login.rs:238`, `crates/server/src/api/rest/endpoints/protocol/forgery.rs:35`) |
| T-LOG-2 | The same, from a browser that says where the post came from | TA-1 | Refused before anything else where the browser says another site started it; absent, the header says nothing and the sealed value is the barrier (`crates/server/src/api/rest/endpoints/protocol/login.rs:117`, `forgery.rs:58`) |
| T-LOG-3 | A password tried against one account as fast as the caller likes, or one password tried against many accounts | TA-1 | Failures counted per address, and per address with the typed name, on every door that verifies a password, on in a stock realm and weighed before the name is looked up, so an address turned away costs one read and no hash (`crates/auth/src/login/browser.rs:200`, `crates/ldapfront/src/lib.rs:285`, `crates/services/src/account/mod.rs:95`); an IPv6 network of 64 bits is one address (`crates/auth/src/login/throttle.rs:223`); at the sign-in page, a browser holding a token sealed for the typed name is counted in its address's place, under the same thresholds (`crates/auth/src/login/browser.rs:184`, `crates/auth/src/login/throttle.rs:129`); lockout per person where the realm turns it on, and spared to that browser. See R-2 |
| T-LOG-8 | A client answering a decoupled sign-in in the person's place, approving what it asked for itself with any token of hers it holds, or deciding what other clients asked of her | TA-4 | Only the person decides, signed in on this realm's own pages or through her account console: a bearer at the doorbell is admitted by the account API's own rule, an access token the account console obtained, carrying the account scope, bound to nothing, for a login still open, so another client's token, a refresh or an identity token, a bound one without its proof or an event told about her answers nothing (`crates/services/src/account/api.rs:176`, `crates/services/src/oidc/ciba.rs:515`, `crates/server/src/api/rest/endpoints/protocol/ciba.rs:437`) |
| T-LOG-15 | A signed decoupled request presented again within its window, ringing the person once more for a request they already answered | TA-4 | The request's identifier is spent as soon as it is read, where the client's assertions are, and committed before anything else is decided, so a request refused for what it asks stays spent, CIBA 7.1.1 (`crates/services/src/oidc/ciba.rs:167`, `crates/server/src/api/rest/endpoints/protocol/ciba.rs:212`) |
| T-LOG-9 | A relying party that demanded a strong login as essential handed a weaker one, the ID token saying so and nothing refusing it | TA-1 | The end of every interactive login, local or brokered, compares what it reached with what its request named as essential and answers `unmet_authentication_requirements` when it falls short, OIDC Core 5.5.1.1 (`crates/services/src/oidc/minting.rs:270`, `crates/models/src/entities/acr.rs:273`) |
| T-LOG-5 | Which names are held, read from when the count turns an address away | TA-1 | Every refusal counts, a name nobody holds and a person the lock refused alike, under the name as typed with case and spacing aside, never under the account it resolves to (`crates/auth/src/login/throttle.rs:13`, `crates/auth/src/login/browser.rs:353`, `crates/ldapfront/src/lib.rs:317`); a device token is read from the token alone before the name is looked up, and one that does not open under the typed name is no token, so holding one says nothing its holder had not proved (`crates/auth/src/login/device.rs:74`) |
| T-LOG-13 | A short code guessed at the device page: a device sign-in somebody else started answered by the guesser, so the device signs in to the guesser's account, or is refused | TA-1 | Eight letters of nineteen, about 34 bits, living ten minutes (`crates/services/src/oidc/device.rs:264`); where the realm counts by address, as a stock realm does, every code that does not stand counts against the address typing it, under a key no name's count shares, and ten misses in fifteen minutes turn the address away before any code is looked up, RFC 8628 §5.1 (`crates/services/src/oidc/device.rs:181`, `crates/auth/src/login/throttle.rs:117`); the count is kept with the refusal (`crates/server/src/api/rest/endpoints/protocol/device.rs:198`) |
| T-LOG-6 | A password typed into the name box, read back out of the failure counts, or out of the notes of a login in progress, in a database dump, or one name matched across realms there | TA-7 | A typed name is kept as a MAC under a key the KEK is expanded to for that use alone, which the database never holds, so the counts read without the KEK test no guess (`crates/auth/src/login/throttle.rs:92`, `crates/crypto/src/envelope.rs:244`, `:113`); the tenant and the realm are keyed with the name, so one name typed in two realms leaves two digests nobody can match without the KEK (`crates/auth/src/login/throttle.rs:255`); a login notes the name it is weighed under in that form, and a device token is sealed under it (`crates/auth/src/login/browser.rs:322`, `:551`); the key is derived once when the process starts and handed to every door, so each counts a name where the others do (`crates/outbound/src/sealing.rs:43`, `crates/saffui/src/main.rs:421`) |
| T-LOG-17 | A decoupled sign-in asked for an address two accounts share, ringing whichever of them the store read first, an account registered under somebody else's address among them | TA-2, TA-4 | An address names its sole holder at the decoupled door, in a plain hint as in a hint token, as it does at the login: one two accounts share names neither, and opens the ghost an unknown hint opens (`crates/services/src/oidc/ciba.rs:442`, `:459`) |
| T-LOG-18 | A door finding a person by address answering for whichever of two accounts sharing it the store read first: a reset link or a subject request's confirmation sent for one of them, a provisioning or directory search answering for one | TA-2, TA-3 | An address stands for its sole holder where a person names themselves by it, at the password reset and the subject door as at the sign-in, and for every holder where a search asks who holds it, at the provisioning door and the directory door; no read of one account by address remains that is not the sole holder's (`crates/store/src/providers/directory/users.rs:88`, `crates/services/src/account/recovery.rs:215`, `crates/services/src/account/privacy.rs:204`, `crates/services/src/scim/users.rs:48`, `crates/ldapfront/src/lib.rs:646`) |
| T-LOG-10 | A single texted code attested as two factors: the phone-first flow, or a code offered as one way in among several, answering a request for a second factor with one | TA-1 | A one-time code reaches the second factor's class only beside something else that passed in the same login, and a single factor's alone; a key keeps its class, its ceremony having the person verify themselves on it; the level a step-up asks for is weighed the way a login passes a flow, so it never names one the login cannot reach (`crates/auth/src/login/authenticator.rs:175`, `:189`, `crates/services/src/account/mod.rs:464`) |
| T-LOG-19 | A realm's bill run up through its codes: one code tried two ways counted twice, or the way asked for back and forth to send a code each time | TA-1 | A code is counted once by the realm's brakes, before it is drawn and whichever way it then goes, the gateway taking it after Meta refused included; the one change of way a login gets without waiting goes only to a way that can carry the code, and the login's own cap and the realm's brakes still hold it (`crates/auth/src/login/authenticator.rs:677`, `:730`, `:651`, `crates/outbound/src/delivery.rs:138`) |
| T-LOG-20 | A code sent to a WhatsApp account its number no longer names: a number the carrier handed on, its former holder's WhatsApp still registered, receives the codes and can prove the number | TA-1 | **Only partly.** A code goes to the number the account proved, and WhatsApp drops an account once its number is registered again on another phone; until then the former holder reads what is sent there, while a text in the same login reaches the SIM's holder (`crates/auth/src/messaging.rs:1017`) |
| T-LOG-21 | A code sent to a SIM the number was moved to: whoever had the carrier issue a new SIM for the number receives the codes it is sent | TA-1 | **Only where the process runs the experimental guard and the realm names its carrier.** The carrier is asked over CAMARA before any code goes, on a token for that one number and an assertion the realm's own key signs for each endpoint; a SIM changed within the realm's window holds the code, which is voided, and the round is played again where the step fails and the flow's other ways answer, a phone's proof left owed; a carrier's silence sends or holds as the realm says; no transaction waits on the carrier (`crates/commons/src/feature.rs:202`, `crates/outbound/src/delivery.rs:87`, `crates/outbound/src/camara.rs:42`, `crates/services/src/token/assertion.rs:21`, `crates/server/src/api/rest/endpoints/protocol/login.rs:356`, `crates/services/src/oidc/sign_in.rs:89`, `crates/auth/src/login/authenticator.rs:617`) |
| T-LOG-11 | A relying party slow to answer its logout notice holding the logout, and every other client's notice, behind it | TA-4 | The notices of an ended login are posted all at once, each with its own five seconds of patience, so a logout waits for the slowest client and never the sum (`crates/server/src/api/rest/endpoints/protocol/backchannel.rs:28`) |
...
| T-LOG-12 | A login ended by an administrator, a reset, a switch-off or the lifecycle rules left alive at every relying party keeping its own session for the person, the very sessions the ending was meant to end | TA-1, TA-2 | Every store function that ends a login owes each of its clients a logout notice in the same transaction, as does taking one application's access back, and the outbox pass sends them with retries under a lease, the lease growing with each attempt; the browser's own logout still tells at once (`crates/store/src/providers/protocol/logout_notices.rs:22`, `:87`, `crates/scheduler/src/logout_notices.rs:20`) |
| T-LOG-7 | The authorization endpoint made to send a browser where no client registered: a link on the provider's own name that lands on a page of somebody else's | TA-1 | A refusal is sent only to a redirect the client registered, matched whole, and shown otherwise; a request object that cannot be read is believed in nothing, its redirect included, so its refusal goes to the redirect beside it only when that one is registered (`crates/services/src/oidc/authorize.rs:840`, `:236`, `:200`) |
| T-LOG-14 | A request object signed for another provider, or dated in a shape the reading skipped, obeyed here as though it were made for this one | TA-1 | An audience of several has to count this issuer among them, a client id stated in another shape names nothing, and a date with a fraction is held to, OIDC Core 6.1 and RFC 7519 2 (`crates/services/src/oidc/request_object.rs:126`, `:138`) |
| T-LOG-16 | A pushed request object never opened, read in the clear from a client that registered encryption, or taken at the push and refused only later in a browser | TA-1, TA-4 | The push judges its object as the authorization endpoint would, and there: opened where the client encrypts and refused in the clear from such a client, then read, RFC 9126 3 (`crates/server/src/api/rest/endpoints/protocol/par.rs:102`, `:131`) |

### The token path, CJ-1

| Id | Threat | Agent | What answers it |
|---|---|---|---|
| T-TOK-1 | A stolen code spent by another client, or against another redirect | TA-1, TA-4 | The code carries the client and the redirect it was minted for, compared at redemption (`crates/services/src/oidc/grant.rs:437`, `:442`) |
| T-TOK-2 | A code spent twice | TA-1 | One atomic spend (`crates/store/src/providers/protocol/oidc.rs:77`); a replay revokes every token that code bought and closes the client session (`crates/services/src/oidc/grant.rs:416`) |
| T-TOK-19 | A client signing people in over a device although its configuration says it may not, a flag reading false taken for one that is present | TA-4 | A device grant flag that says no opts nobody in, and any other presence still does (`crates/services/src/oidc/device.rs:35`) |
| T-TOK-3 | A public client's code intercepted in the browser | TA-1 | A challenge is required of a public client, S256 only (`crates/services/src/oidc/authorize.rs:772`, `:775`) |
| T-TOK-4 | The proof key stripped in flight | TA-1 | A verifier presented for a code carrying no challenge is refused whoever the client is (`crates/services/src/oidc/grant.rs:675`) |
| T-TOK-5 | A refresh token stolen and renewed | TA-1, TA-4 | Rotation by default, compared and rotated in one write, and a replay closes the family while leaving the sign in alive (`crates/services/src/oidc/grant.rs:1506`, `crates/store/src/providers/protocol/sessions.rs:547`, `crates/services/src/oidc/grant.rs:1560`) |
| T-TOK-15 | One person holding more offline grants than the realm allows, each standing long after any login, by collecting them over a device | TA-2 | Every grant that writes an offline grant makes room under the realm's cap, the device poll as the code and decoupled grants, ending the oldest (`crates/services/src/oidc/grant.rs:1269`) |
| T-TOK-6 | A bearer token replayed by whoever holds it | TA-1 | Binding compared at presentation and at renewal, and a token naming two bindings satisfies both (`crates/services/src/token/mod.rs:202`) |
| T-TOK-20 | A DPoP proof read off the wire presented again after the request that spent it was refused | TA-1, TA-6 | The spend of a proof is committed as soon as the proof is verified, at the token endpoint and the push as at userinfo, so a refusal after it leaves it spent (`crates/server/src/api/rest/endpoints/protocol/token.rs:172`, `crates/server/src/api/rest/endpoints/protocol/par.rs:184`) |
| T-TOK-7 | Algorithm confusion, or an unsecured token | TA-4 | No unsecured variant exists to select, and the verifying algorithm comes from the stored key rather than the token's header (`crates/services/src/token/mod.rs:102`) |
| T-TOK-14 | An identity token a client registered to receive encrypted handed out readable by the device or the decoupled grant, for whoever reads the answer on its way or wherever the client keeps it | TA-6 | Every grant that mints an identity token wraps it for a client that registered encryption, the device and decoupled polls as the code grant, and one that cannot be encrypted to is refused rather than answered in the clear (`crates/services/src/oidc/grant.rs:1057`, `:1280`, `crates/services/src/oidc/encryption.rs:120`) |
| T-TOK-8 | A public client acting as a machine | TA-4 | Refused, with the same face as a client that never opted in (`crates/services/src/oidc/grant.rs:196`) |
| T-TOK-18 | An exchange let through unasked because the policy meant to decide it was named in part, or in a shape nobody reads | TA-4 | A policy named in part, or with a part nobody can read, refuses the exchange; named whole it is asked, named not at all it is no policy (`crates/services/src/oidc/grant.rs:1863`) |
| T-TOK-9 | Delegation grown wider or deeper than it was given | TA-4 | Scope intersected and never widened, a ceiling of five links, and a bound token cannot be exchanged (`crates/services/src/oidc/grant.rs:1961`, `:1831`, `:1753`) |
| T-TOK-17 | Delegation kept alive past the token it acts on, a token about to end exchanged for a fresh one and the result exchanged again | TA-4 | What an exchange mints ends no later than the token it was shown, and a token with nothing left buys nothing (`crates/services/src/oidc/grant.rs:2031`) |
| T-TOK-10 | A revoked authority still spending | TA-2, TA-4 | Realm wide and per client cuts are read at every door that takes a token (`crates/services/src/token/mod.rs:254`, `:267`) |
| T-TOK-16 | An exchange pointed at any audience although the operator bounded it, the bound written as a list or in a shape nobody reads | TA-4 | The bound is read in every shape it is written in, a list as the string it spells, and one that names nobody allows nobody but the client itself (`crates/services/src/oidc/grant.rs:1981`) |
| T-TOK-11 | A person switched off still signed in through the logins she held: her browser minting codes at every client, a code or a token from before still honoured, and all of it revived when she is switched back on | TA-2 | Every write that switches a person off ends every login she holds, offline grants included (`crates/services/src/directory.rs:24`); and a login stands only while its person is switched on, read in the same statement as the login, at the token gate behind every door, at the reuse of a signed-in browser and at the code exchange (`crates/store/src/providers/protocol/sessions.rs:98`, `crates/services/src/token/mod.rs:295`, `crates/services/src/oidc/authorize.rs:748`, `crates/services/src/oidc/grant.rs:459`) |
| T-TOK-12 | A token lifted off a DPoP or mutual-TLS client spent at a resource server that validates by introspection, told a bearer token with no binding to compare | TA-1, TA-6 | The introspection answer carries the token's `cnf` whole and calls a token bound to a key DPoP, RFC 9449 6.2 and RFC 8705 3.2; the resource server holding the proof compares it (`crates/services/src/oidc/introspection.rs:82`, `:89`) |
| T-TOK-23 | A DPoP proof whose `iat` runs ahead of the server's clock, presented again once its spend was swept away while that `iat` still stood in the window | TA-1, TA-6 | A spent proof is kept until its `iat` has left the window on every clock that reads it: a window past the `iat` rather than past the moment it was spent, and a drift beyond for another instance's clock and for the database's that sweeps it (`crates/services/src/client/dpop.rs:172`) |
| T-TOK-22 | A key-bound token read at userinfo under the bearer scheme, or an unbound one under DPoP's, the scheme saying one thing and the token another | TA-1 | The scheme a token arrives under has to agree with what it is bound to, and a caller speaking DPoP is refused in DPoP's words, RFC 9449 7.1 and 7.2 (`crates/services/src/oidc/userinfo.rs:170`) |
| T-TOK-13 | An agent's client switched off, or a realm that closed the exchange, still minting capability tokens at the MCP door | TA-4 | The MCP door answers to the realm's exchange switch as the token endpoint does, at the handshake as at the first mint, and a client switched off exchanges nothing there, as it authenticates nowhere else (`crates/server/src/api/rest/endpoints/protocol/mcp.rs:108`, `:241`) |
| T-TOK-21 | A client held to keys it withdrew or rotated, its answers encrypted to a key it no longer publishes and its request objects verified under old ones, or its host asked again and again by requests that fail | TA-4, TA-1 | The keys a client publishes are read before each use that needs them once what was kept is thirty seconds old; the reading is claimed on a transaction of its own committed at once, so a refused request does not undo it and the host is asked once a keeping, and no transaction is open while the host answers (`crates/outbound/src/egress.rs:168`, `crates/store/src/providers/clients/mod.rs:199`, `crates/server/src/api/rest/endpoints/protocol/caller.rs:95`) |
...
| T-ADM-9 | A relation store nobody opened, or one a realm closed, still deciding: an edge written before it closed walked at the decision door or in a simulation, a share accepted that no walk will read, a realm told it opened what the process does not run | TA-2, TA-3 | The store is experimental and runs only in a process started with it (`crates/commons/src/feature.rs:192`). Closed, its doors refuse (`crates/server/src/middleware/admin_guard.rs:236`), every relationship question answers `relation-store-closed` before any walk (`crates/services/src/authorization/pdp.rs:337`), a simulation walks nothing for its trace (`crates/server/src/api/rest/endpoints/admin/authorization.rs:660`), a share is refused in words while taking one back is not (`crates/services/src/admin/authorization.rs:491`), and a realm asking to open what the process does not run is refused in words (`crates/server/src/api/rest/endpoints/admin/features.rs:136`) |

### Tenancy, CJ-2

| Id | Threat | Agent | What answers it |
|---|---|---|---|
| T-TEN-1 | A query that forgets its filter reading another realm | TA-3 | Policies on two keys, forced, on 99 tables (`crates/store/src/tenancy.rs:17`) |
...
| R-2 | The lockout per person is off in a stock realm, and the count per address sees one address at a time, as the deployment believes it | A password tried once against many accounts from many addresses trips nothing; a deployment that believes a forwarded address from any peer lets a caller name a new one per attempt (T-EDGE-3) (`crates/models/src/entities/realm.rs:43`, `:102`). People who share one address share its count on the directory front and at a password change, and at the sign-in page from a browser that has not signed in there under the name it types. Once it has, a token sealed under that name has the browser counted in its address's place, turned away past the threshold for one name, and spared the lock per person (`crates/auth/src/login/browser.rs:184`, `:547`, `crates/auth/src/login/device.rs:74`, `crates/auth/src/login/mod.rs:121`, `crates/server/src/api/rest/endpoints/protocol/binding.rs:38`). Whoever lifts that token out of the browser gets the same, for that one name and from any address, until it lapses 90 days after the sign-in that minted it (`crates/auth/src/login/device.rs:24`) |
