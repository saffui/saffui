...
use crate::jose::{JoseError, Map, Number, Value};
use anyhow::bail;
...
mod tests {
    ...
    fn test_f64_time_payload() -> Result<()> {
        ...
        assert!(
            matches!(payload.claim("exp"), Some(Value::Number(val)) if val == &Number::from_f64(1.000000001).unwrap())
        );
        assert!(
            matches!(payload.claim("nbf"), Some(Value::Number(val)) if val == &Number::from_f64(12.000000012).unwrap())
        );
        assert!(
            matches!(payload.claim("iat"), Some(Value::Number(val)) if val == &Number::from_f64(123.000000123).unwrap())
        );
        assert!(
