  ...
  <div>
    ...
    <div v-if="editorOpen" class="mt-4 rounded-lg border border-border bg-surface p-4">
      ...
      <form class="mt-3 grid gap-3 sm:grid-cols-2" @submit.prevent="save">
        ...
        <label class="text-[11px] font-medium text-muted sm:col-span-2">{{ say("mappers-col-type") }} <AppHint name="mappers-col-type-help" /><select v-model="draft.mapper_type" class="sf-field mt-1 font-mono"><option v-for="type in types" :key="type" :value="type">{{ type }}</option></select></label>
        <label class="text-[11px] font-medium text-muted sm:col-span-2">{{ say("mappers-config") }} <AppHint name="mappers-config-help" /><textarea v-model="draft.configs" rows="6" class="sf-field mt-1 font-mono text-[11px]" spellcheck="false" /></label>
