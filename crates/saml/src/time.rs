...
pub(crate) fn read_instant(text: &str) -> Option<i64> {
...
pub(crate) fn write_instant(seconds: i64) -> Option<String> {
...
#[cfg(test)]
mod tests {
    use super::{read_instant, write_instant};

    /// A written time reads back to the same second, and a second no calendar
    /// holds is not written.
    #[test]
    fn a_written_time_reads_back() {
        let written = write_instant(1_789_372_800).expect("a time");
        assert_eq!(written, "2026-09-14T08:00:00Z");
        assert_eq!(read_instant(&written), Some(1_789_372_800));
        assert_eq!(write_instant(i64::MAX), None);
    }
}
