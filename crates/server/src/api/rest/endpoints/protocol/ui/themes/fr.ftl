...
policy-min-length = Au moins { $n } caractères
policy-max-length = Au plus { $n } caractères
policy-digits = Au moins { $n } chiffres
policy-upper = Au moins { $n } majuscules
policy-lower = Au moins { $n } minuscules
policy-special = Au moins { $n } caractères spéciaux
policy-not-username = Différent de votre identifiant
policy-not-email = Différente de votre adresse e-mail
...
flash-throttled = Trop de tentatives échouées depuis ce navigateur ou ce réseau. La connexion depuis cet endroit est suspendue un moment ; réessayez plus tard.
...
whatsapp-sent = Un code vous a été envoyé sur WhatsApp. Saisissez-le ci-dessous.
whatsapp-code = Code reçu sur WhatsApp
code-not-received = Pas reçu ?
code-by-sms = Recevoir par SMS
code-by-whatsapp = Recevoir sur WhatsApp
