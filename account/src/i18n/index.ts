...
const PINNED_KEY = "sf-account-tongue";
...
function chooseTongue(): string {
  ...
  for (const asked of globalThis.navigator?.languages ?? []) {
...
const tongue = chooseTongue();

export function readTongue(): string {
  return tongue;
}

export function listTongues(): string[] {
  return TONGUES.map(([held]) => held);
}

/// Pin a tongue and reload: the bundles are chosen at boot, and a reload is honest
/// about that rather than half-translating the open page.
export function pinTongue(chosen: string): void {
  try {
    localStorage.setItem(PINNED_KEY, chosen);
  } catch {
...
/// A message by name, in the console's tongue, falling back to the first tongue
/// and then to the name itself, so a missing string is visible and never a blank.
export function say(name: string, args?: Record<string, string | number>): string {
  for (const held of [bundles.get(tongue), bundles.get(TONGUES[0][0])]) {
    ...
    if (held && message?.value) return held.formatPattern(message.value, args);
  }
  return name;
}
